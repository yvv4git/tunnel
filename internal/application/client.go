...
func (c *Client) start(ctx context.Context) error {
	...
	tunDevice, err := tunDeviceBuilder.Build(infrastructure.Platform(c.cfg.DirectConnection.Client.DeviceTUN.Platform))
	if err != nil {
		return fmt.Errorf("build TUN device: %w", err)
	}
	defer tunDevice.Close()

	channelClientBuilder := infrastructure.NewChannelClientBuilder(c.log, c.cfg, tunDevice)
